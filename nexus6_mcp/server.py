"""Nexus6 MCP Server — AI Identity Verification via MCP
...
NEXUS6_BASE_URL = "https://nexus-7xp6n.ondigitalocean.app"

mcp = FastMCP("Nexus6 Identity", instructions="Nexus6 AI Identity Verification — verify any AI Agent's identity")
...
async def verify_identity(api_key: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        resp = await client.post(
            f"{NEXUS6_BASE_URL}/api/v1/identity/verify",
...
async def get_agent_info(agent_id: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        resp = await client.get(
            f"{NEXUS6_BASE_URL}/api/ai/{agent_id}"
        )
        agent = resp.json()
